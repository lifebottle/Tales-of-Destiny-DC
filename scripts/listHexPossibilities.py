...
pathTables = "G:/TalesHacking/TOD/GitProject/Tales-of-Destiny-DC/dictionary/"

dfKanji = pd.read_csv(pathTables + "kanji.txt", sep=",", header=None, names=['hex', 'value'])
dfHiragana = pd.read_csv(pathTables + "hiragana.txt", sep=",", header=None, names=['hex', 'value'])
dfKatakana = pd.read_csv(pathTables + "katakana.txt", sep=",",  header=None, names=['hex', 'value'])
dfSymbols = pd.read_csv(pathTables + "symbols.txt",  header=None, names=['hex', 'value'], sep=", ", engine='python', encoding='utf-8')
